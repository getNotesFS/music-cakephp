...
{
    public $entity_name = 'artist';
    public $entity_name_plural = 'artists';
    ...
    public $table_buttons = [
        ...
        'Borrar' => [
            ...
            'options' => [
                'confirm' => '¿Está seguro de que desea eliminar el artista?',
                'class' => 'button'
            ]
        ]
    ];
    ...
    public $paginate = [
        'limit' => 20,
        'order' => [
            'Artists.published_date' => 'DESC',
            'Artists.published_time' => 'DESC'
        ]
    ];
    ...
    public function add()
    {
        ...
        $discographies = $this->{$this->getName()}->Discographies->find('list');

        if ($entity->getErrors()) {
            $this->Flash->error(__('No se ha podido guardar el álbum. Por favor, intente nuevamente.'));
        }

        $this->set(compact('entity', 'discographies'));
    }
